...
class CaseDetailParser:
    ...
    def parties(self):
        ...
        for span in party_div.find_all(class_=re.compile("tyler-span")):
            ...
            if 'Plaintiff' in span.get_text():
                party_dict['party_type'] = 'Plaintiff'
                party_dict['party_name'] = ' '.join(name_elem.text.replace('Plaintiff', '').split())
            if 'Defendant' in span.get_text():            
                party_dict['party_type'] = 'Defendant'
                party_dict['party_name'] = ' '.join(name_elem.text.replace('Defendant', '').split())
            if 'Respondant' in span.get_text():            
                party_dict['party_type'] = 'Respondant'
                party_dict['party_name'] = ' '.join(name_elem.text.replace('Respondant', '').split())
